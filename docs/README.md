...
| [analysis-mode.md](analysis-mode.md) | `server/lib/stockfish.ts`, `server/routes/analyze.ts`, `client/src/pages/Analysis.tsx` | Stockfish UCI subprocess lifecycle, score normalization (White's perspective), movetime-based search, analysis caching, SSE streaming protocol, auto-start analysis, `useDeferredValue` for Recharts, precomputed eval data |
...
| [ui-components.md](ui-components.md) | `client/src/components/EvalBar.tsx`, `client/src/components/EvalGraph.tsx`, `client/src/components/MoveList.tsx`, `client/src/components/GameCard.tsx` | EvalBar score clamping and display, EvalGraph with Recharts (memoized inflection points, deferred currentMove, click navigation), MoveList with precomputed classification strings and React.memo, auto-scroll |
