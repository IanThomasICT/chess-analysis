...
_(subnote: keys of different types are allowed `[cow=4,cow=4.0,cow(),cow=moo]`,_
_but it’s better not to know about that)._
...
- **max\_batch=<int>** (default: 1024) Maximum size of minibatch that
...
- **gpu=<int>** (default: 0) Which GPU device to use (0-based: 0 for the
first GPU device, 1 for the second, etc).
- **force\_tune=true** Triggers search for the best configuration for GPU.
- **tune\_only=true** Force exits the engine as soon as GPU tuning is
complete.
- **tune\_exhaustive=true** Tries more configurations during tuning. May take
...
- **blas\_cores=<int>** (default: 1) Number of cores to use (probably?..)
- **batch\_size=<int>** (default: 256) Maximum size of minibatch that
...
backend.
- **max\_batch=<int>** (default: 256) Maximum size of batch to send to this
backend.
...
[◃◃◃ Announcing LcZ, the world's first neural net based chess engine and interactive fiction amalgamation.](https://lczero.org/blog/2019/04/announcing-lcz-worlds-first-neural-net/) [Competition Season! ▹▹▹](https://lczero.org/blog/2019/05/competition-season/)