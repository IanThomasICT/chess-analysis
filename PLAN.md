...
- [x] **Explorer (D29)** — built. `server/lib/explorer.ts` (`positionFrequency(fenKey, move)` lazy read-only SQLite lookup + `normalizeSan` + `setExplorerDbPath` test seam; returns `null` when the table is absent → ECO fallback preserved byte-for-byte). `server/scripts/build-explorer.ts` (`bun run explorer <dump.pgn.zst> [--url] [--out] [--min-elo] [--max-elo] [--max-ply] [--min-samples] [--max-games]`): streams via `Bun.file().stream().pipeThrough(new DecompressionStream("zstd"))`, band-filters 1100–2200 + standard-only, strips eval/clk comments + NAGs, replays ≤24 plies → `fen_key → move → count`, prunes `< min-samples`, then flushes `game_metrics_ext`+`game_metrics` for clean recompute (D26). Wired into `game-metrics.ts` via `scanOutOfBook` (explorer-first, D33 ECO fallback); no `METRICS_VERSION` bump (absent-explorer values identical). `server/data/explorer/` gitignored. Tests: `explorer.test.ts` (5), `build-explorer.test.ts` (16), +3 `game-metrics.test.ts` out-of-book cases. ✅ `bun run validate` green, **460 unit tests pass**. Synthetic CLI smoke verified (band filter, comment strip, transposition collapse). The real multi-GB dump build remains a manual run (Risk 3).
