...
export function RecurrencePanel({ fen, username, currentGameId }: RecurrencePanelProps) {
  ...
  return (
    <div>
      <div className="mb-2 text-sm font-semibold text-fg">Position recurrence</div>
        ...
        {filtered.map((entry) => {
          ...
          return (
            <Link
              ...
              className="flex items-center gap-2 rounded px-2 py-1 text-xs hover:bg-raised"
            >
              {entry.was_blunder ? (
                <span className="text-blunder" title="blundered">&#9679;</span>
              ) : (
                <span className="text-faint" title="no blunder">&#9675;</span>
              )}
